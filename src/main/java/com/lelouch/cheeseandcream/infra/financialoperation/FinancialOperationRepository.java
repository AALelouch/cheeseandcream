...
public interface FinancialOperationRepository extends JpaRepository<FinancialOperationEntity, Long> {
    ...
    @Query("SELECT SUM(t.total) FROM FinancialOperationEntity t WHERE t.active = true and t.creationDate BETWEEN :startDate AND :endDate and t.operationType = 'SALE'")
