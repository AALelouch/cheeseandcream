...
function EditInvoice() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [agentsRes, productsRes, invoiceRes] = await Promise.all([
          agentApi.getAll(),
          productApi.getAll(),
        ...
        setSelectedAgent(invoice.agentId?.toString() || '')
        setInvoiceType(invoice.invoiceType || 'SALE')
        setHasBalance(invoice.balance > 0)
        setBalanceAmount(invoice.balance > 0 ? invoice.balance.toString() : '')

        if (invoice.products && invoice.products.length > 0) {
          setInvoiceProducts(invoice.products.map(p => ({
            productId: p.productId?.toString() || '',
            quantity: p.quantity || 1
          })))
        }
      } catch (err) {
        setError('Error al cargar datos de la factura')
        console.error(err)
      } finally {
        setLoading(false)
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Error al actualizar la factura: ' + (err.response?.data?.message || err.message))
      console.error(err)
    } finally {
      setSubmitting(false)
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
          <button type="submit" className="btn btn-primary" disabled={submitting}>
            {submitting ? 'Actualizando...' : 'Actualizar Factura'}
          </button>
